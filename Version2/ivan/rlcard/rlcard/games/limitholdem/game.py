...
class LimitholdemGame(object):
    ...
    def run(self):
        """
        finish the game out and output result
        """
        tmp = self.get_player_id()
        while not self.is_over():
            # print(state, " run game")
            self.step(random.choice(self.get_legal_actions()))

        # print(self.get_payoffs(), self)
        return self.get_payoffs()[tmp]
