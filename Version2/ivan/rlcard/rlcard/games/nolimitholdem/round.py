...
class NolimitholdemRound():
    ...
    def get_nolimit_legal_actions(self, players):
        ...
        if self.dealer.pot >= player.remained_chips or self.dealer.pot == abs(players[(self.game_pointer + (self.num_players - 1)) % self.num_players].in_chips - player.in_chips):
            full_actions.remove(Action.RAISE_POT)

        if int(self.dealer.pot / 2) >= player.remained_chips or self.dealer.pot//2 == abs(players[(self.game_pointer + (self.num_players - 1)) % self.num_players].in_chips - player.in_chips):
