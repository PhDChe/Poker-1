...
class Worker(Process):
            ...
            if command == "KillProc":
                ...
                if args is None:
                    ...
                    if first_action is not None:
                        state, reward = self.wrapped_env.step(first_action)

                if first_action is not None and state['terminal']:
                    accu_reward = reward
                else:
                ...
                if first_action is not None:
                    self.send_safe_protocol("ReturnSimulation", (task_idx, accu_reward, reward, state['moves'] == []))
                else:
                    self.send_safe_protocol("ReturnSimulation", (task_idx, accu_reward, prior_prob))

    def expand_node(self, checkpoint_data, curr_node):
        # print(checkpoint_data, '_________________________________________________________________')
        ...
        next_state, reward = self.wrapped_env.step(expand_action)
        ...
        return expand_action, next_state, reward, next_state.is_over(), checkpoint_data
    ...
    def simulate(self, state, max_simulation_step = 100, lamda = 0.5):
        ...
        while not done and step_count < max_simulation_step:
            ...
            next_state, reward = self.wrapped_env.step(action)
