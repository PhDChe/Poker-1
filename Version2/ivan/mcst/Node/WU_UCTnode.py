...
class WU_UCTnode():
    ...
    def update_incomplete(self, idx):
        ...
        action_taken = [Action.FOLD, Action.CHECK, Action.CALL, Action.RAISE_HALF_POT, Action.RAISE_POT, Action.ALL_IN].index(action_taken)
    ...
    def add_child(self, action, child_state, checkpoint_idx, prior_prob = None):
        action = [Action.FOLD, Action.CHECK, Action.CALL, Action.RAISE_HALF_POT, Action.RAISE_POT, Action.ALL_IN].index(action)
        if self.children[action] is not None:
