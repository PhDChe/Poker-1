...
class WU_UCT():
    ...
    def simulate_trajectory(self, pokerstate, max_episode_length = -1):
        state = pokerstate
        self.wrapped_env.restore(state)
        accu_reward = 0.0
        done = False
        step_count = 0
        rewards = []
        ...
        while not done and (max_episode_length == -1 or step_count < max_episode_length):
            ...
            # print('wassa')
            # Interact with the environment
            next_state = self.wrapped_env.step(action)
    ...
    def simulate_single_move(self, state):
        ...
        return best_action, self.root_node

    def simulate_single_step(self, sim_idx):
        # Go into root node
        curr_node = self.root_node
        # print(self.checkpoint_data_manager.buffer, 'data buffer')
        # Selection
        curr_depth = 1
        while True:
            print(curr_node.checkpoint_idx, 'checkpoint index for selection ' + str(curr_depth), str(sim_idx))
            if curr_node.no_child_available() or (not curr_node.all_child_visited() and curr_node == self.root_node):
                ...
                # print(checkpoint_data, 'game state for selection - '+ str(curr_depth))
                # print('hi')
                # Record the task
        ...
        if not need_expansion:
            ...
            if self.expansion_worker_pool.server_occupied_rate() >= 0.99:
                ...
                curr_node.update_history(task_idx, expand_action)
                action_taken = [Action.FOLD, Action.CHECK, Action.CALL, Action.RAISE_HALF_POT, Action.RAISE_POT, Action.ALL_IN].index(expand_action)
                curr_node.dones[action_taken] = done
                curr_node.rewards[action_taken] = reward
